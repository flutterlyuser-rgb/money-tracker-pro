...
class _BalancePageState extends ConsumerState<BalancePage> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (paymentAccounts.isNotEmpty) ...[
                    ...
                    ...paymentAccounts
                        .map((acc) => _buildAccountRow(context, acc, isOther: false))
                        .toList(),
                    const SizedBox(height: 20),
                  ],
                  ...
                  if (otherAccounts.isNotEmpty) ...[
                    ...
                    const SizedBox(height: 4),
                    ...otherAccounts
                        .map((acc) => _buildAccountRow(context, acc, isOther: true))
                        .toList(),
                    const SizedBox(height: 20),
                  ],
                  // Additional content
  ...
  Widget _buildAccountRow(BuildContext context, AccountEntity account, {required bool isOther}) {
    ...
    // Build the row content separately so it can be wrapped with a tap handler.
    ...
    // When not editing, wrap the row in an InkWell to navigate to the account page.
    if (!_isEditing) {
      return InkWell(
        onTap: () {
          // Only navigate if the account has an id. This prevents navigation for
          // accounts that are not yet persisted (e.g. being created).
          if (account.id != null) {
            Navigator.push(
              context,
              MaterialPageRoute(
