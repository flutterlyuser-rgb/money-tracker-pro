...
class _BalancePageState extends ConsumerState<BalancePage> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (paymentAccounts.isNotEmpty) ...[
                    ...
                    // Use a reorderable list when in edit mode. Otherwise, map
                    ...
                    _isEditing
                        ...
                        : Column(
                            children: paymentAccounts
                                .map((acc) => _buildAccountRow(
                                      context,
                                      acc,
                                      isOther: false,
                                      indexInSection: paymentAccounts.indexOf(acc),
                                    ))
                                .toList(),
                          ),
                    const SizedBox(height: 20),
                        ...
                        : Column(
                            children: otherAccounts
                                .map((acc) => _buildAccountRow(
                                      ...
                                      acc,
                                      isOther: true,
                                      indexInSection: otherAccounts.indexOf(acc),
                                    ))
                                .toList(),
                          ),
                    const SizedBox(height: 20),
  ...
  /// Builds a reorderable section for a given list of accounts.  When
  /// editing we wrap the rows in a [ReorderableListView] to allow
  /// dragging items up or down using a handle.  When dragging, only
  /// rows within the section will move; the overall order of the list
  /// will be updated via the provider.
  Widget _buildReorderableSection({required List<AccountEntity> accounts, required bool isOther}) {
    return ReorderableListView.builder(
      ...
      onReorder: (int oldIndex, int newIndex) {
        // When dragging downwards the newIndex will include the removed item
        // so we subtract one to get the destination index within the list.
        if (newIndex > oldIndex) {
          newIndex -= 1;
        }
        // Determine the global indices of the accounts being moved within
        // the provider's list. We build a list of indices for the
        // relevant section and then map the reorder indices back to the
        // underlying provider.
        final allAccounts = ref.read(accountsProvider);
        final List<int> sectionIndices = [];
        for (int i = 0; i < allAccounts.length; i++) {
          final a = allAccounts[i];
          final bool matches = isOther ? !a.trackIncome : a.trackIncome;
          if (matches) {
            sectionIndices.add(i);
          }
        }
        if (oldIndex < 0 || oldIndex >= sectionIndices.length ||
            newIndex < 0 || newIndex >= sectionIndices.length) {
          return;
        }
        final int fromIndex = sectionIndices[oldIndex];
        final int toIndex = sectionIndices[newIndex];
        ref.read(accountsProvider.notifier).reorderAccounts(fromIndex, toIndex);
      },
    );
  }

  /// Builds an individual account row. When editing the row includes
  /// delete and reorder controls; otherwise it simply shows the
  /// account details. Tapping an account navigates to the
  /// appropriate page (edit page in edit mode or transaction page
  /// otherwise). The [isOther] flag determines which icon to display
  /// (wallet for payment accounts, home for other assets). The
  /// [indexInSection] is used when editing to provide the correct
  /// index for the drag handle.
  Widget _buildAccountRow(BuildContext context, AccountEntity account,
      {required bool isOther, required int indexInSection}) {
    // Determine the appropriate icon based on account type.
    ...
    // Determine the index of this account within the provider's list
    // so we can enable/disable reorder controls correctly.
    ...
    Widget rowContent = Row(
      children: [
        ...
        Text(
          formattedBalance,
          style: TextStyle(
            color: account.currentBalance < 0 ? const Color(0xFF00BCD4) : const Color(0xFF4CAF50),
            fontSize: 16,
            fontWeight: FontWeight.w500,
          ),
        ),
        if (_isEditing) ...[
          const SizedBox(width: 8),
          // Drag handle for reordering. We use ReorderableDragStartListener to
          // initiate the drag. The index passed here should correspond to
          // the position within the section, not the global list.
    ...
    // Wrap row in a GestureDetector to handle taps. In edit mode we
    // navigate to the account edit page; otherwise we navigate to
    ...
    return Padding(
      ...
      child: GestureDetector(
        onTap: () {
          if (_isEditing) {
            // Determine a type for editing based on trackIncome.
            final AccountType editType = account.trackIncome ? AccountType.payment : AccountType.asset;
            Navigator.push(
              context,
              MaterialPageRoute(
