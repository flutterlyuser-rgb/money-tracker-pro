...
class AccountsNotifier extends StateNotifier<List<AccountEntity>> {
  ...
  /// Reorders the in‑memory list of accounts.
  ///
  /// When the user is editing the Balance page they can move
  /// accounts up or down. This method swaps the account at
  /// [oldIndex] to [newIndex] in the current state list. Note
  /// that this change is only applied to the provider state and
  /// does not persist any ordering in the database. The UI will
  /// rebuild with the new order automatically.
  void reorderAccounts(int oldIndex, int newIndex) {
    final List<AccountEntity> updated = List.from(state);
    if (oldIndex < 0 || oldIndex >= updated.length) return;
    if (newIndex < 0 || newIndex >= updated.length) return;
    final account = updated.removeAt(oldIndex);
    updated.insert(newIndex, account);
    state = updated;
  }
}
